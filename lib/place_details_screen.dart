...
class _PlaceDetailsScreenState extends State<PlaceDetailsScreen> {
  ...
  Future<void> _addMorePhotos() async {
    ...
    try {
      // withData: true is REQUIRED on Android/iOS, otherwise file.bytes is
      // null on those platforms and every picked photo gets silently
      // skipped below (this was the cause of "gallery photos not added").
      ...
      _showMessage('Failed to open gallery: $e', isError: true);
      return;
    }
    ...
    try {
      ...
      for (int i = 0; i < result.files.length; i++) {
        ...
        // Detect the real image MIME type so the original bytes are
        // served back with the correct Content-Type instead of the
        // generic binary type uploadBinary defaults to.
        String mimeType = lookupMimeType(file.name) ?? 'image/jpeg';
      ...
      if (newImageUrls.isNotEmpty) {
        ...
        _showMessage('Added ${newImageUrls.length} image(s) • ${totalMb.toStringAsFixed(1)} MB uploaded at full quality.');
      }
      if (skipped > 0) {
        _showMessage('$skipped photo(s) could not be read and were skipped.', isError: true);
      } else if (newImageUrls.isEmpty) {
        _showMessage('No images were added.', isError: true);
      }
    } catch (e) {
              ...
              children: [
                if (_images.isNotEmpty)
                  _PhotoGallery(
                    ...
                    deletingPhoto: _deletingPhoto,
                    onTapImage: _showFullScreenImage,
                    onDeleteCurrent: _deleteCurrentImage,
                    onIndexChanged: (index) => _currentImageIndex = index,
                  ),
...
/// Self-contained photo gallery. Swiping only rebuilds this small widget
/// (via its own State), never the parent PlaceDetailsScreen — which
/// previously re-ran the entire page build (reviews list, rating bars,
/// info cards) on every single swipe and made swiping feel stuck/laggy
/// on real devices. Also adds tap arrows as a reliable fallback in case
/// a drag gesture gets swallowed by the outer scroll view.
class _PhotoGallery extends StatefulWidget {
  final List<String> images;
  final bool isLoggedIn;
  final bool deletingPhoto;
        ...
        children: [
          PageView.builder(
            ...
            itemBuilder: (context, index) => GestureDetector(
              onTap: () => widget.onTapImage(widget.images[index]),
              child: Hero(
          ...
          if (hasMultiple) ...[
            // Tap-to-navigate arrows — a reliable backup if a swipe ever
            // gets intercepted by the page's outer scroll view.
            if (_currentIndex > 0)
              Positioned(
                left: 8,
                top: 0,
                bottom: 0,
                child: Center(
                  child: _NavArrow(icon: Icons.chevron_left_rounded, onTap: () => _goTo(_currentIndex - 1)),
                ),
              ),
            if (_currentIndex < widget.images.length - 1)
              Positioned(
                ...
                bottom: 0,
                child: Center(
                  child: _NavArrow(icon: Icons.chevron_right_rounded, onTap: () => _goTo(_currentIndex + 1)),
                ),
              ),
            Positioned(
              bottom: 54,
              ...
              child: Row(
                ...
                children: List.generate(
                  widget.images.length,
                    ...
                    height: 6,
                    decoration: BoxDecoration(
                      color: i == _currentIndex ? Colors.white : Colors.white.withOpacity(0.5),
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Text('${_currentIndex + 1} / ${widget.images.length}', style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
            ),
          ),
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: IconButton(
                  ...
                  onPressed: widget.deletingPhoto ? null : widget.onDeleteCurrent,
                ),
              ),
            ),
        ],
